import React from 'react';
import { Card, Image, Text, Button, Group, Badge }  from "@mantine/core";
import {ProductCardProps} from "../../types/types.ts";

const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
    const [quantity, setQuantity] = React.useState(1);

    return (
        <Card
            shadow="sm"
            padding="16px"
            style={{
                width: '302px',
                height: '414px',
                opacity: 1,
                gap: '16px',
                display: 'flex',
                flexDirection: 'column',
                backgroundColor: '#FFFFFF',
                border: '2px solid red',
                borderRadius: '24px',
            }}
        >
            <Card.Section style={{ padding: '16px 10px 0 16px' }}>
                <Image
                    src={product.image}
                    height={276}
                    width={276}
                    alt={product.name}
                    style={{
                        opacity: 1,
                        margin: '0 auto',
                        borderRadius: '8px',
                        border: '2px solid green',
                    }}
                />
            </Card.Section>

            {/* Первая строка: Название товара и количество */}
            <Group
                style={{
                    width: '270px',
                    height: '30px',
                    opacity: 1,
                    display: 'flex',
                    justify: 'space-around',
                    border: '2px solid blue',
                    margin: '0 16px',
                }}
            >
                <Text
                    fw={600}
                    size="lg"
                    style={{
                        lineHeight: '1.2'
                    }}
                >
                    {product.name}
                </Text>

                {/* Кастомный счетчик с текстовыми кнопками +/- и Badge */}
                <Group
                    style={{
                        display: 'flex',
                    }}>
                    <Button
                        variant="outline"
                        size="xs"
                        style={{
                            width: '30px',
                            height: '30px',
                        }}
                    >
                        -
                    </Button>

                    <Badge
                        variant="filled"
                        color="gray"
                        size="lg"
                        style={{
                            width: '12px',
                            height: '24px',
                            margin: '3px 9px'
                        }}>
                        {quantity}
                    </Badge>

                    <Button
                        variant="outline"
                        size="xs"
                        style={{
                            width: '30px',
                            height: '30px',
                        }}
                    >
                        +
                    </Button>
                </Group>
            </Group>

            {/* Вторая строка: Цена и кнопка добавления в корзину */}
            <Group
                style={{
                    width: '270px' ,
                    height: '44px',
                    display: 'flex',
                    justify: 'space-around',
                    border: '2px solid yellow',
                    margin: '0 16px',
                }}>
                <Text
                    fw={700}
                    size="xl"
                    style={{
                        lineHeight: '1.2',
                        width: '54px',
                        height: '24px',
                        margin: '10px 12px 10px 0',
                    }}
                >
                    ${product.price}
                </Text>

                <Button
                    variant="filled"
                    onClick={() => {
                        // Добавляем товар quantity раз
                        for (let i = 0; i < quantity; i++) {
                            onAddToCart(product);
                        }
                        // Сбрасываем количество после добавления
                        setQuantity(1);
                    }}
                    styles={{
                        root: {
                            width:'204px',
                            height: '44px',
                            border: 'none',
                            borderRadius: '8px',
                            fontSize: '16px',
                            fontWeight: 600,
                            backgroundColor: '#b9e1c2',
                        }
                    }}
                >
                    <Text
                        styles={{
                            root: {
                                width: '87px',
                                height: '24px',
                                fontSize: '16px',
                                fontWeight: 600,
                                color: '#2a7a3f',
                                margin: '10px 10px 10px 43,5px',
                            }
                        }}>
                        Add to cart
                    </Text>
                        {/*<svg*/}
                        {/*    xmlns="http://www.w3.org/2000/svg"*/}
                        {/*    width="19"*/}
                        {/*    height="19"*/}
                        {/*    fill="none"*/}
                        {/*    style={{ margin: '12px 43,5px 12px 0' }}>*/}
                        {/*    <path fill="#3b944e" d="M1.5.66a.833.833 0 1 0 0 1.667zm1.91.834.805-.218A.83.83 0 0 0 3.41.661zm3.105 11.463-.804.217c.108.401.495.661.907.61zm9.552-1.194.104.826a.83.83 0 0 0 .712-.658zM17.5 4.837l.816.169a.833.833 0 0 0-.816-1.002zm-13.184 0-.804.218zM1.5 2.327h1.91V.661H1.5zm5.118 11.457 9.553-1.195-.207-1.653-9.552 1.194zm10.265-1.853 1.433-6.925-1.632-.338-1.433 6.926zM2.606 1.711l.906 3.344 1.608-.436-.905-3.343zm.906 3.344 2.199 8.12 1.608-.436-2.199-8.12zM17.5 4.004H4.316V5.67H17.5zm-8.833 12.08c0 .23-.187.416-.417.416v1.667c1.15 0 2.083-.933 2.083-2.084zm-.417.416a.417.417 0 0 1-.417-.417H6.167c0 1.15.932 2.084 2.083 2.084zm-.417-.417c0-.23.187-.416.417-.416V14c-1.15 0-2.083.933-2.083 2.083zm.417-.416c.23 0 .417.186.417.416h1.666c0-1.15-.932-2.083-2.083-2.083zm7.083.416c0 .23-.186.417-.416.417v1.667c1.15 0 2.083-.933 2.083-2.084zm-.416.417a.417.417 0 0 1-.417-.417h-1.667c0 1.15.933 2.084 2.084 2.084zm-.417-.417c0-.23.187-.416.417-.416V14c-1.15 0-2.084.933-2.084 2.083zm.417-.416c.23 0 .416.186.416.416H17c0-1.15-.933-2.083-2.083-2.083z"/>*/}
                        {/*</svg>*/}
                </Button>
            </Group>
        </Card>
    );
};

export default ProductCard;